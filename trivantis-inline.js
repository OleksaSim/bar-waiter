  ...
  if (arguments.length>1) {
	...
	for(var i=0; i<this.parmArray.length; i++)
	{
		...
		if( currParm.length > 6 && currParm.indexOf('<embed') == 0)
			this.parmArray[i] = "<embed src='" + varValue + "' width='" + this.w + "'   height='" + this.h + "' type='application/x-shockwave-flash' pluginspage='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash' name='" + this.name + "' swliveconnect='true'>";
	}
      }
      this.build();
      var str = "";
...
function ObjInlineActionMute() {
    if (this.iType == 'flashvid') {
        ...
        if (this.plyVer == 8 && !this.muteState) this.flsPlayer.sendEvent('volume', 0);
        else this.flsPlayer.sendEvent('MUTE', true);
...
function ObjInlineActionUnmute() {
    if (this.iType == 'flashvid') {
        ...
        if (this.plyVer == 8 && this.muteState) this.flsPlayer.sendEvent('volume', 80);
        else this.flsPlayer.sendEvent('MUTE', false);
        this.muteState = false;
    }
}
